...
├── .github/
│   ├── workflows/
│   │   └── ci.yml          # Esteira de CI/CD (SAST, SCA e Validação)
│   └── dependabot.yml      # Atualização autônoma de dependências e Actions
├── assets/
│   ├── css/
│   │   └── styles.css      # Estilização desacoplada
│   └── js/
│       └── script.js       # Lógica criptográfica e controle do DOM
├── eslint.config.mjs       # Configuração de Linter/SAST estático
├── index.html              # Interface de usuário com CSP aplicada
├── package.json            # Gestão de dependências de dev (Linters/Scanners)
├── package-lock.json       # Trava de versões exatas para Builds Reprodutíveis
└── README.md
```
